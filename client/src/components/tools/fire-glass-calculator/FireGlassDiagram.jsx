// Simple illustrative 3D outline of the selected fire pit shape, showing
// which dimensions (length/width/side/base/height/diameter + depth) feed
// into the area and volume formulas.

const EDGE   = "#2563eb";
const HIDDEN = "#94a3b8";
const LBL    = "#1f2937";

const DX = 40, DY = -30; // extrusion offset (depth direction, drawn back-right)
...
export default function FireGlassDiagram({ shape = "rectangular" }) {
  return (
    <svg viewBox="0 0 300 190" style={{ width: "100%", height: "auto", display: "block" }}
      ...
      {(shape === "rectangular" || shape === "square") && (
        <>
          ...
          <Label x={130} y={168}>{shape === "square" ? "s" : "l"}</Label>
          <Label x={222} y={98}>{shape === "square" ? "s" : "w"}</Label>
          <Label x={202} y={112}>d</Label>
        </>
      )}
      ...
      {shape === "triangular" && (
        <>
          ...
          <Label x={222} y={98}>d</Label>
        </>
      )}

      {shape === "trapezoidal" && (
        <>
          <Prism front={[[55, 150], [185, 150], [155, 70], [85, 70]]} />
          <Label x={95} y={64}>a</Label>
          <Label x={120} y={168}>b</Label>
          <Label x={68} y={112}>h</Label>
          <Label x={222} y={98}>d</Label>
        </>
      )}
    </svg>
