...
import {
  calculateFromBalance, formatCurrency, formatWholeCurrency, formatYearsAndMonths,
  formatYearsMosShort, formatPercentWhole,
} from "../../../utils/mortgagePayoffCalculatorEngine";

const DEFAULTS = {
  unpaidPrincipal: "230000", monthlyPayment: "1500", annualRatePercent: "6",
...
export default function MortgagePayoffFromBalanceCard() {
  ...
  function calculate() {
    const r = calculateFromBalance({
      unpaidPrincipal: unpaidPrincipal || DEFAULTS.unpaidPrincipal,
  ...
  function clear() {
    setUnpaidPrincipal(""); setMonthlyPayment(""); setAnnualRatePercent("");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <p style={{ fontSize: 13.5, color: "var(--text-secondary)", lineHeight: 1.6, margin: 0 }}>
        Use this calculator if the term length of the remaining loan is not known. The unpaid principal balance,
            ...
            <div style={{ padding: "14px 16px" }}>
              {!result ? (
                ...
                <>
                  ...
                  <div style={rowStyle}><span style={{ color: "var(--text-secondary)" }}>Remaining term</span><span>{formatYearsMosShort(result.remainingTotalMonths)}</span></div>
                  <div style={rowStyle}><span style={{ color: "var(--text-secondary)" }}>Total Payments</span><span>{formatCurrency(result.remainingTotalPayments)}</span></div>
                  <div style={{ ...rowStyle, borderBottom: "none" }}><span style={{ color: "var(--text-secondary)" }}>Total Interest</span><span>{formatCurrency(result.remainingTotalInterest)}</span></div>
                </>
              ) : (
                <>
                  <p style={{ fontSize: 13.5, color: "var(--text-secondary)", lineHeight: 1.6, marginBottom: 16 }}>
                    The remaining term of the loan is <strong>{formatYearsAndMonths(result.remainingTotalMonths)}</strong>. By paying{" "}
                    ...
                    <strong style={{ color: "var(--success)" }}>{formatCurrency(result.remainingTotalInterest - result.newTotalInterest)}</strong> in interest.
                  </p>

                  <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
                    <SavingsBox
                      title="Interest savings"
                      titleValue={formatWholeCurrency(result.remainingTotalInterest - result.newTotalInterest)}
                      originalLabel={formatWholeCurrency(result.remainingTotalInterest)}
                      payoffLabel={formatWholeCurrency(result.newTotalInterest)}
                      originalValue={result.remainingTotalInterest}
                      payoffValue={result.newTotalInterest}
                      percentLabel={`Pay ${formatPercentWhole((result.remainingTotalInterest - result.newTotalInterest) / result.remainingTotalInterest * 100)} less on interest`}
                    />
                    <SavingsBox
                      title="Time savings"
